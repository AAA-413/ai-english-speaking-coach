...
async function createDeepSeekSummary({
  ...
  const model = env.DEEPSEEK_TEXT_MODEL || "deepseek-chat";
  const baseUrl = (env.DEEPSEEK_BASE_URL || "https://api.deepseek.com").replace(/\/$/, "");
  const maxTokens = positiveInteger(env.DEEPSEEK_MAX_TOKENS, 8000);
  const retryMaxTokens = positiveInteger(env.DEEPSEEK_RETRY_MAX_TOKENS, Math.max(12000, maxTokens));

  let payload = await requestDeepSeekSummary({
    baseUrl,
    apiKey: env.DEEPSEEK_API_KEY,
    fetchImpl,
    body: buildDeepSeekSummaryBody({ model, scenario, level, turns, maxTokens }),
  });

  let text = extractDeepSeekOutputText(payload);
  if (!text) {
    payload = await requestDeepSeekSummary({
      baseUrl,
      apiKey: env.DEEPSEEK_API_KEY,
      fetchImpl,
      body: buildDeepSeekSummaryBody({
        model,
        scenario,
        level,
        turns,
    ...
    text = extractDeepSeekOutputText(payload);
  }
...
async function requestDeepSeekSummary({ baseUrl, apiKey, fetchImpl, body }) {
...
function buildDeepSeekSummaryBody({ model, scenario, level, turns, maxTokens, retry = false }) {
  return {
    model,
    messages: [
      {
        ...
        content: [
          ...
          "Keep Chinese explanations short and actionable.",
          "Pick at most three high-value corrections.",
          retry ? "Use compact JSON. Do not include markdown, code fences, or extra commentary." : "",
        ].filter(Boolean).join("\n"),
      },
      {
        role: "user",
